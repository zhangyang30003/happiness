...
X_train, X_val, Y_train, Y_val = train_test_split(X, Y, test_size = 0.0, shuffle=True, random_state=42)
...
#            (0.1 * svr_model_full_data.predict(X_train)) + \
            ...
            (0.25 * lightgbm_model_full_data.predict(X_train)) + \
...
#Y = (Y - Y.mean()) / Y.std()
y_pred_test_blend_fixed = y_pred_test_blend * Y.std() + Y.mean()
...
pd.DataFrame(SUB_RESULT).to_csv('comp_reg_sk_v4_long.csv', index = 0, header = 1)   
